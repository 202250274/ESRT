...
class Un(nn.Module):
    def __init__(self,n_feats, wn, upscale=2, conv=common.default_conv):
        ...
        self.down = nn.AvgPool2d(kernel_size=2)
        scale = upscale
        self.upin = nn.Sequential(common.Upsampler(conv,scale,n_feats,act=False),
    ...
    def forward(self,x):
        ...
        out = x3
        b,c,h,w = x3.shape
        out = self.down(self.reduce(torch.cat([x1,x2,x3],dim=1)))
        b,c,h,w = out.shape
